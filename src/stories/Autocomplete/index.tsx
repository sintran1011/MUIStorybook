...
const Autocomplete = forwardRef<HTMLInputElement, AutocompleteProps>(
  (props, ref) => {
    ...
    const [optionValueArr, setOptionValueArr] = useState<
      MenuItemProps[] | null
    >([]);
    const [searchKey, setSearchKey] = useState<string | undefined>("");
    const [memoryOptions, setMemoryOptions] =
      useState<MenuItemProps[]>(extraOptions);
    ...
    const handleOptionArrChange = (
      _: unknown,
      option: MenuItemProps[] | null,
    ) => {
    ...
    const filterOptions = createFilterOptions({
      stringify: (option: MenuItemProps) =>
        option[filterOptionsDisplay || "value"],
    });
      ...
      >
        <MUIAutocomplete
          ...
          onChange={(
            ...
            if (!multiple)
              return handleOptionChange(_, option as MenuItemProps);
            ...
            const removeExistValue = existValue
              ? controlledValuesArray?.filter(
                  (i) => i.value !== clickedValue?.value,
                )
              : option;
            ...
            return value.map((option, index: number) => {
              ...
              return (
                <Chip
                  ...
                  color="brand"
                  onDelete={
                    readOnly || disabled ? undefined : chipProps.onDelete
                  }
                />
              );
            });
