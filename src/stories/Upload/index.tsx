...
const Upload = (props: UploadProps) => {
  ...
  const colorText = disabled ? 'dark.300' : 'black';
  ...
  const handleDropFile = (file: File[], rejectFile: FileRejection[]) => {
    const isOverMaxFile = rejectFile.some(i => i.errors[0].code === 'too-many-files');
    if (isOverMaxFile) toast.warning(`You can only upload maximum ${maxFiles} files`);
  ...
  const renderBeforeUpload = () => {
    if (customSteps && customSteps.beforeUpload) return customSteps.beforeUpload();
    return (
      <>
        {/* <img src={FileUploadIcon} /> */}
        <Stack justifyContent="center" alignItems="center" gap="8px">
          <Typography fontSize="14px" lineHeight="20px" color={colorText} variant="body-medium">
            Drag & drop
          </Typography>
          <Divider
            sx={{
          ...
          >
            <Typography color={colorText} fontSize="14px" lineHeight="20px" variant="body-medium">
              or
            </Typography>
          </Divider>
          <Button variant="contained" color="brand" disabled={disabled}>
            <Typography color="#ffffff" variant="body-strong-medium">
              Select File
            </Typography>
          </Button>
        </Stack>
  ...
  const renderProgressLoading = (progress?: number) => {
    ...
    return (
      <Stack justifyContent={'space-between'} alignItems={'center'} width={'100%'} height={'100%'}>
        <Stack gap={'4px'} alignItems={'center'} justifyContent={'center'}>
          {/* <FileUploadIcon /> */}
          <Typography color="white" fontSize={'14px'} fontWeight={600} lineHeight={'18px'}>
            Uploading...
          </Typography>
        </Stack>
        <Stack width={'100%'} height={'100%'} alignItems={'end'} justifyContent={'end'} gap={'4px'}>
          <Typography color="white" fontSize={'14px'} fontWeight={600} lineHeight={'18px'}>
            {progress}%
          </Typography>
          <LinearProgress
  ...
  const renderAfterUpload = (values: FileUpload[]) => {
    if (customSteps && customSteps.afterUpload) return customSteps.afterUpload(values);
    return (
      <>
        <Stack gap={2} justifyContent="center" alignItems="center">
          {/* <img src={FileUploadDoneIcon} /> */}
          <Typography fontSize="14px" lineHeight="20px" color="white" variant="body-medium">
            <Typography fontSize="14px" lineHeight="18px" fontWeight="400" component="span">
              {values && values.length > 1 ? `${values.length.toString()} files` : 'File'}
            </Typography>{' '}
